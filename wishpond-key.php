  ...
  */
  class WishpondKey
  {
    ...
    */
    ...
    {
      $master_token = WishpondKey::get_master_token();

      $new_auth_token = WishpondAuthenticator::request_auth_token( $master_token );

      self::auth_token( $new_auth_token );

      if( $new_auth_token != false && $new_auth_token != '' )
      {
        self::auth_token_expiry( (int)time() + (int)WISHPOND_FACEBOOK_ADS_AUTH_TOKEN_TTL );
      }
    ...
    */
    public static function create_master_token( $email = "" )
    {
      ...
      $auth_token = WishpondAuthenticator::request_auth_token( $master_token );
    ...
    */
    public static function auth_token( $auth_token = "" )
    {
      if( $auth_token == "" && $auth_token !== false )
      {
        ...
        if( self::$auth_token == '' )
        {
          self::$auth_token = WishpondStorage::get( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN );
        }
      }
      else
      {
        // setter
        WishpondStorage::delete( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN );
        if( $auth_token != false )
        {
          WishpondStorage::add( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN, $auth_token );
        }
        self::$auth_token = $auth_token;
      }
      return self::$auth_token;
    ...
    public static function auth_token_expiry( $auth_token_expiry = "" )
    {
      if( $auth_token_expiry == "" && $auth_token_expiry !== false)
      {
        ...
        {
          self::$auth_token_expiry = WishpondStorage::get( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN_EXPIRY );
        }
      }
      else
      {
        WishpondStorage::delete( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN_EXPIRY );
        if( $auth_token_expiry != false )
        {
          WishpondStorage::add( WISHPOND_FACEBOOK_ADS_AUTH_TOKEN_EXPIRY, $auth_token_expiry );
        }
    ...
    */
    public static function master_token( $master_token = "" )
    {
      if( $master_token == "" && $master_token !== false )
      {
        ...
        {
          self::$master_token = WishpondStorage::get( WISHPOND_FACEBOOK_ADS_MASTER_TOKEN );
        }
      }
      else
      {
        // setter
        WishpondStorage::delete( WISHPOND_FACEBOOK_ADS_MASTER_TOKEN);
        if( $master_token != false )
        {
          WishpondStorage::add( WISHPOND_FACEBOOK_ADS_MASTER_TOKEN, $master_token );
        }
        self::$master_token = $master_token;
      }
    ...
    private static function random_key()
    {
      ...
      $hashed_string .= WishpondHelpers::get_random_string( 64 );
